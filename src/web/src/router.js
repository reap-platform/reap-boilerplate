...
function getRouterData (app) {
  ...
  functions.forEach((aFunction) => {
    routerData[aFunction.code] = {
      ...
      // eslint-disable-next-line
      component: connect(aFunction.connect ? aFunction.connect : state => ({ ...state[aFunction.code], context: aFunction ,loading: state.loading.models[aFunction.code],effects: state.loading.effects }))(require(`./routes/${aFunction.name ? aFunction.name : funcCode.code}.js`)),
    }
  })
  return routerData
}
